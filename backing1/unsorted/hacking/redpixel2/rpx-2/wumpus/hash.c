 ...
 */
void hash_enumerate(HASH_TABLE * table, void (*callback)(char *, void *))
{
    unsigned int i;
    /*BUCKET *temp;*/
     ...
     * (replaced by the simpler `do ... while' above)
     * 
     * It worked under DOS, under Windows, but under Linux (my distribution
     * at least) the data inside a struct would be corrupted as soon it was
     * free'd (perhaps by another process).  Therefore when control returned
     * from `callback' (pointing to `free_node') it would segfault at the 
     * point `temp = temp->next'.  Phew.
     */
#if 0    
