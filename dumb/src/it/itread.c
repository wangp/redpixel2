...
*/
		...
		if (sample->flags & IT_SAMPLE_16BIT) {
			if (convert & 2) {
				for (n = 0; n < size; n++) {
					sample->left[n] = (int)(signed short)dumbfile_mgetw(f);
					sample->right[n] = (int)(signed short)dumbfile_mgetw(f);
				}
			} else {
				for (n = 0; n < size; n++) {
	...
	if ((sample->flags & (IT_SAMPLE_LOOP | IT_SAMPLE_SUS_LOOP)) &&
...
static int it_read_pattern(IT_PATTERN *pattern, DUMBFILE *f)
{
	...
	while (bufpos < buflen) {
		...
		{
			static unsigned char used[16] = {0, 1, 1, 2, 1, 2, 2, 3, 2, 3, 3, 4, 3, 4, 4, 5};
			n_entries += (mask != 0);
			bufpos += used[mask & 15];
		}
	}
	...
	for (n = 0; n < n_components; n++) {
		...
		switch (component[n].type) {
			...
			case IT_COMPONENT_PATTERN:
				if (it_read_pattern(&sigdata->pattern[component[n].n], f)) {
					_dumb_it_unload_sigdata(sigdata);
					return NULL;
				}
				break;
