...
#include <allegro.h>
#include <libnet.h>		/* XXX */
#include "fps.h"
#include "gameloop.h"
#include "gamenet.h"		/* XXX */
#include "magic4x4.h"
#include "net.h"		/* XXX */
#include "render.h"
#include "store.h"		/* XXX */
...
void game_loop (int server)
{
    ...
    if (server) {
	player = object_create ("player", OBJECT_ROLE_AUTHORITY);
	player->x = map->starts.next->x;
	player->y = map->starts.next->y;
	map_link_object (map, player);
	...
	t = tick;
	while (t > 0) {
	    if (key[KEY_LEFT])  camera.x--;
	    if (key[KEY_RIGHT]) camera.x++;
	    if (key[KEY_UP]) 	camera.y--;
	    if (key[KEY_DOWN])  camera.y++;
	    if (key[KEY_Q]) 	quit = 1;

	    if (key[KEY_W]) try_jump (player);
	    else jump = 100;
	    if (key[KEY_A])  try_left (player);
	    if (key[KEY_D]) try_right (player);
